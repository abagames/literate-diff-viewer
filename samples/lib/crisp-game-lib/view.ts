...
export function setParentElement(e: HTMLElement) {
  parentElement = e;
}
...
`;
  ...
  const setSize = () => {
    const width =
      parentElement == null ? innerWidth : parentElement.clientWidth;
    const height =
      parentElement == null ? innerHeight : parentElement.clientHeight;
    const cs = 0.95;
    const wr = width / height;
    const cr = canvasSize.x / canvasSize.y;
    const flgWh = wr < cr;
    const cw = flgWh ? cs * width : cs * height * cr;
    const ch = !flgWh ? cs * height : (cs * width) / cr;
