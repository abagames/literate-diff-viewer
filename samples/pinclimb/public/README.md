...
Take a look at the template on the right side of the screen. `title` and `description` are variables to set the game title and description, `characters` are variables to define the pixel art to be displayed on the screen, and `options` are variables to set the game settings. These variables will be set later.
...
On this page, to make it easier to understand the game behavior even when the mouse cursor is outside the game screen, the state in which the mouse button is pressed is reproduced at random. The cord automatically stretches and shrinks with this reproduction action.
...
In this game, the pins scroll down from the top of the screen, so if the cord is at the top of the screen, it is difficult to see what is happening beyond the screen. Therefore, if the y-coordinate of the pin at the center of the cord is less than 80, the scrolling distance is increased.
...
On this page, the sound will only be played when the mouse cursor hovers over the game screen. Move the mouse cursor over the game screen, press the button to extend the cord, and confirm that the sound effect is heard when the cord moves.
